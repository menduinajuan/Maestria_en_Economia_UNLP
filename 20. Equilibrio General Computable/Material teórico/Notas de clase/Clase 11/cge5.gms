...
*$CALL GDXXRW user-files\arg2012-data.xlsx index=layout!A1
*$GDXIN arg2012-data.gdx

*$CALL GDXXRW user-files\cri2012-data.xlsx index=layout!A1
*$GDXIN cri2012-data.gdx

*$CALL GDXXRW user-files\ecu2007-data.xlsx index=layout!A1
*$GDXIN ecu2007-data.gdx

*$CALL GDXXRW user-files\per2007-data.xlsx index=layout!A1
*$GDXIN per2007-data.gdx

*$CALL GDXXRW user-files\pry2012-data.xlsx index=layout!A1
*$GDXIN pry2012-data.gdx

$CALL GDXXRW user-files\col2016-data.xlsx index=layout!A1
$GDXIN col2016-data.gdx
...
*++
  tq0(c)            rate of sales tax for commodity c
...
*!!
  pwe0(c)          export price for c (foreign currency)
  pwm(c)           import price for c (foreign currency)
*!!
  pwm0(c)           import price for c (foreign currency)
  te(c)            export subsidy rate for commodity c
  tm(c)            import tariff rate for commodity c
*++  
  tm0(c)            import tariff rate for commodity c
...
* initial values for endog and exgog variables, and parameters 
...
pwe('c-agr') = pwe0('c-agr')*1.25;

SOLVE CGE5 USING CNS;

$BATINCLUDE reploop.inc pweagr
...
pwm('c-agr') = pwm0('c-agr')*1.25;

SOLVE CGE5 USING CNS;

$BATINCLUDE reploop.inc pwmagr
