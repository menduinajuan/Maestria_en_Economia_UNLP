...
This model is based on Cicowiez and Lofgren (2013), which in turn descends from CGE5 in Lofgren (2003).
...
PARAMETERS
  ...
  QINVSCAL0          factor de ajuste de la demanda de inversión
  ...
  ty                 tasa de impuesto sobre el ingreso de los hogares
...
EQ_SAVINV..
  SUM(c, PQ(c) * QINV(c)) + SUM(c, PQ(c) * qdstk(c)) + WALRAS =E= mps * (1 - ty) * YH + (YG - EG) + savf * EXR;
...
DISPLAY QQ.L, QA.L, QX.L, QINT.L, QH.L, EG.L, QINV.L, QE.L, QM.L, LD.L, KD.L,
        PQ.L, PA.L, PX.L, CPI.L, EXR.L, WL.L, WK.L, YH.L, YL.L, YK.L, trnsfr, WALRAS.L;
...
* 2. Duplicar la oferta de trabajo

*LS.FX = LS0 * 2;

* 3. Duplicar el consumo gubernamental de ambos productos

*qg(c) = qg(c) * 2;

* 4. Eliminar los aranceles a la importación

*tm(c) = 0;

* 5. Aumentar 15% el precio mundial de exportación del producto "c-prv"
