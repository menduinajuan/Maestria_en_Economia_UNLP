* Equilibrio General Computable - Trabajo Práctico N° 4 (Ejercicio 3)
* Juan Menduiña
...
$CALL GDXXRW cri2012-data.xlsx index=layout!A1
...
errsambalchk(ac)$(ABS(sambalchk(ac)>1e-10)) = 1/0;
...
PARAMETERS
  ...
  qg0(c)             demanda del gobierno del producto c
...
DISPLAY EG0, YG0, SAVG0, SAVGR0, qg, qg0, trnsfr;
...
* 1. Aumento del 15% en el precio mundial del principal producto de exportación ('c-othsvc')

$INCLUDE var+prm-init.inc
pwe('c-othsvc') = pwe0('c-othsvc') * 1.15;
SOLVE CGE USING CNS;
$BATINCLUDE reploop.inc pwe

* 2. Aumento del 10% en el consumo público

$INCLUDE var+prm-init.inc
qg(c) = qg0(c) * 1.1;
SOLVE CGE USING CNS;
$BATINCLUDE reploop.inc qg