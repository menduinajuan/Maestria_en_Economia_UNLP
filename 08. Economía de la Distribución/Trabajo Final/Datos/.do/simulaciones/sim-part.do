...
table gender ocup [iw=fex_c] if (sampsel==1), replace
...
forvalues i=1(1)$n_gender {
	...
	forvalues j=1(1)$n_ocup {
		...
		summarize fex_c if (sampsel==1 & gender==`i'), meanonly
		local obstot=r(sum)
		display as text "obtot = " as result `obstot'

		gsort -sampsel gender ocup -pr`j' rnd

		generate shrpop=sum(fex_c) if (sampsel==1 & gender==`i' & ocup==`j')
...
forvalues i=1(1)$n_gender {
	...
	forvalues j=1(1)$n_ocup {
		...
		if m_sim[`i',`j']>m_obs[`i',`j'] {
			...
			generate shrpop=sum(fex_c) if (sampsel==1 & gender==`i')
			summarize shrpop, meanonly
...
tabulate gender ocup_sim [iw=fex_c] if (sampocup==1), row

preserve

table gender ocup_sim [iw=fex_c] if (sampsel==1), replace
...
do "$dofiles/simulaciones/genera-ing-laboral" "`nombre'"
