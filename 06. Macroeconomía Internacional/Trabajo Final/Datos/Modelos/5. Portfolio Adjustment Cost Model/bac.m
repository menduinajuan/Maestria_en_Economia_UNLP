function [nfx,nfxp,nfy,nfyp] = bac
%This program computes a log-linear approximation to the  function f for a small open economy with portfolio adjustment costs. The model is described in ``Closing Small Open Economy Models,'' by Stephanie Schmitt-Grohe and Martin Uribe. The function f defines  the DSGE model (a p denotes next-period variables) : 
%  E_t f(yp,y,xp,x) =0. 
%
%Inputs: none
%
...
%
%Calls: anal_deriv.m num_eval.m bac_ss.m
%
...
e2 = -log(tb) + yy - c - iv - PHI/2 * (kp -k)^2 - PSSI/2 * (DBAR -dp)^2;

e3 = -yy + a * k^ALFA *  h^(1-ALFA);

e4 = -iv + kp - (1-DELTA) *k;

e5 = - la * (1 + PSSI * (DBAR-dp)) + lap;
...
[GAMA,DELTA,ALFA,PSSI,RHO,OMEGA,PHI,R,DBAR,c,cp,h,hp,k,kp,k1,k1p,d,dp,iv,ivp,tb,tbp,la,lap,yy,yyp,a,ap,tby,tbyp,cay,cayp]=bac_ss;
